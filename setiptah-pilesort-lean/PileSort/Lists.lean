/-!
  ...
  Key results:
  - `list_split_at` — decompose `l = prefix ++ [a] ++ suffix` at the first occurrence of `a`
  - `list_split_two` — decompose `l` at two elements in order
  - `indexOf_filter_lt` — order in a filtered list reflects order in the original
  - `indexOf_reverse_lt` — order reversal under `List.reverse`
  - `flatMap_filter_perm` — partitioning into buckets and flatMapping is a permutation
-/
